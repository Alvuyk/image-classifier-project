...
def display(image_path,probs, classes):   
    processed_image = process_image(image_path)

    label_map = {v: k for k, v in model.class_to_idx.items()}

    classes = [cat_to_name[label_map[c]] for c in classes]
...
def main():
   ...
   # image_tensor = process_image(args.image)
    ...
    probs, labels = predict(image_tensor, model, cat_to_name)
    
    
    display(image_tensor, top_flowers, top_probs)
